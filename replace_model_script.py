#!/usr/bin/env python3
"""
Script to replace your current model with the NaN-safe version
Run this to eliminate all NaN warnings
"""
...
def backup_and_replace():
    """Backup current model and replace with safe version"""
    
    # Check if current model exists
    current_model = Path("multimodal_pk_yolo.py")
    
    if current_model.exists():
        # Create backup
        backup_path = Path("multimodal_pk_yolo_backup.py")
        shutil.copy2(current_model, backup_path)
        print(f"✅ Backed up current model to {backup_path}")
    
    # The safe model code from the artifact above
    safe_model_code = '''"""
...
'''
    ...
    # Write the safe model
    with open("multimodal_pk_yolo.py", "w") as f:
        f.write(safe_model_code)
    
    print("✅ Replaced model with NaN-safe version")
    print("\nTo test the new model:")
    print("python -c \"from multimodal_pk_yolo import test_model; test_model()\"")
    
    print("\nTo resume training with the safe model:")
    print("python training_script.py --data_dir ./data --batch_size 4 --lr 1e-4")

if __name__ == "__main__":
    backup_and_replace()
    print("\n🎉 MODEL REPLACEMENT COMPLETE!")
    print("The NaN warnings should now be eliminated.")
    print("You can continue training with the same command you were using.")